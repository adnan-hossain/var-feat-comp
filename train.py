...
IMAGENET_DIR = Path('/data/datasets/imagenet')
...
def imcls_evaluate(model: torch.nn.Module, testloader):
    ...
    for imgs, labels in pbar:
        ...
        if hasattr(model, 'self_evaluate'):
            ...
            forward_ = getattr(model, 'forward_cls', model.forward)                     # why do it like this????
            p = forward_(imgs)
            # reduce to top5
...
def get_config():
    ...
    parser.add_argument('--wbgroup',    type=str,  default='ours')
    parser.add_argument('--wbmode',     type=str,  default='online')
    # model setting
    parser.add_argument('--model',      type=str,  default='ours_condautoencoder')
    parser.add_argument('--model_args', type=str,  default='')
    # resume setting
    parser.add_argument('--resume',     type=str,  default='ours_condautoencoder_10')
    ...
    # parser.add_argument('--aux_lr',     type=float,default=0.01)
    ...
    parser.add_argument('--amp',        action=argparse.BooleanOptionalAction, default=True)                # what is argparse.BooleanOptionalAction
...
class TrainWrapper():
    ...
    def set_optimizer_(self):
        ...
        if cfg.optimizer == 'sgd':
            ...
            # aux_optimizer = torch.optim.SGD(aux_parameters, lr=cfg.aux_lr, momentum=cfg.momentum)
        elif cfg.optimizer == 'adam':
            optimizer = torch.optim.Adam(parameters, lr=cfg.lr)
            # aux_optimizer = torch.optim.Adam(aux_parameters, lr=cfg.aux_lr)
        elif cfg.optimizer == 'adamw':
            optimizer = torch.optim.AdamW(parameters, lr=cfg.lr)
            # aux_optimizer = torch.optim.AdamW(aux_parameters, lr=cfg.aux_lr)
        else:
        ...
        # self.aux_scaler = amp.GradScaler(enabled=cfg.amp)
    ...
    def set_logging_dir_(self):
        ...
        if cfg.resume: # resume
            ...
            # self.aux_scaler.load_state_dict(checkpoint['aux_scaler'])
            start_epoch = checkpoint['epoch']
            prev_result = checkpoint.get('results', None)
            prev_loss = prev_result['loss_min'] if prev_result is not None else prev_loss
            if self.is_main:
    ...
    def set_ema_(self):
        ...
        if cfg.ema:
            warmup = cfg.ema_warmup_epochs * len(self.trainloader)          # no. of steps to do warmup
            decay = 0.9999
            ema = timm.utils.ModelEmaV2(timm.utils.unwrap_model(self.model), decay=decay, device=self.device)
            ...
            if cfg.resume:
                ...
                ema.module.load_state_dict(torch.load(ckpt_path)['model'])
    ...
    def main(self):
        ...
        if self.is_main:
            ...
            # self.stats_table = SimpleTable(['Epoch', 'GPU_mem', 'lr', 'aux_lr'])
        ...
        for epoch in range(self._start_epoch, cfg.epochs):
            ...
            timm.utils.unwrap_model(model).bottleneck_layer.train()
            # timm.utils.unwrap_model(model).train()
            for bi, (imgs, labels) in pbar:
                niter = epoch * len(self.trainloader) + bi
                # print(timm.utils.unwrap_model(model).layer1.training)
                ...
                with amp.autocast(enabled=cfg.amp):
                    ...
                    # aux_loss = aux_loss / float(cfg.accum_num)
                ...
                if niter % cfg.accum_num == 0:
                    ...
                    if self.ema is not None and self.is_main:
                        self.ema.update(timm.utils.unwrap_model(model))
    ...
    def adjust_lr_(self, epoch):
        ...
        # if cfg.lr_sched == 'cosine':
        #     lrf = get_cosine_lrf(epoch, 0.001, cfg.epochs-1)
        # else:
        #     raise NotImplementedError()

        # for param_group in self.aux_optimizer.param_groups:
        #     param_group['lr'] = cfg.aux_lr * lrf
    ...
    def logging(self, pbar, epoch, bi, niter, imgs, stats):
        ...
        # aux_lr = self.aux_optimizer.param_groups[0]['lr']
        # self.stats_table['aux_lr'] = aux_lr
        ...
        if niter % 100 == 0:
            ...
            _log_dic = {
                'general/lr': cur_lr,
                # 'general/aux_lr': aux_lr,
                # 'ema/n_updates': self.ema.updates if cfg.ema else 0,
                'ema/decay': self.ema.decay if cfg.ema else 0                 # can't find ema.get_decay()
            }
            _log_dic.update(
    ...
    def evaluate(self, epoch, niter):
        ...
        _log_dic.update({'metric/plain_val_'+k: v for k,v in results.items()})          # dict {'top1':top1_acc_value, 'top5':top5_acc_value}
        self.current_acc = results['top1_min']
        # save last checkpoint
        if self.current_acc > self.prev_acc:
            checkpoint = {
        ...
        if self.cfg.ema:
            ...
            if self.current_acc_ema > self.prev_acc_ema:
                ...
                self.prev_acc_ema = self.current_acc_ema
